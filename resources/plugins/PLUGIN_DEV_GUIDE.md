...
**6. `countdown` (倒计时：目标时间 -> 剩余时长)**
*   用途：把 API 返回的未来时间（如下次任务执行时间）转换为"距离现在还有多久"，适合任务调度、维护窗口等场景。
*   输入要求：源变量为可被 .NET 解析的时间字符串，推荐 ISO 8601 格式（如 `2025-07-20T12:34:56+08:00`），带时区偏移时会按本机时间正确计算。
*   可选配置：
    *   `format`：输出模板，默认 `"{hh}:{mm}:{ss}"`。
    *   `fallback`：源变量为空或时间无法解析时显示的文本，默认 `"--:--:--"`。不写该字段与配置空字符串 `""` 语义不同：前者使用默认占位符，后者输出空内容。
*   格式占位符（双写表示补零到两位）：

| 占位符 | 含义 | 示例 |
| :--- | :--- | :--- |
| `{d}` | 天数（仅当模板中包含 `{d}` 时，小时按天内剩余小时计算） | `2` |
| `{h}` / `{hh}` | 小时（不补零 / 补零） | `5` / `05` |
| `{m}` / `{mm}` | 分钟（不补零 / 补零） | `3` / `03` |
| `{s}` / `{ss}` | 秒（不补零 / 补零） | `9` / `09` |
| `{tm}` | 总分钟数（整数，可用于 `threshold_switch` 做阈值判断） | `75` |

*   示例：把 `next_time` 显示为 `时:分:秒`
```json
{
  "var": "remaining",
  "source": "next_time",
...
```
*   行为说明：
    *   目标时间已过时，按零值渲染（默认格式显示 `00:00:00`）。
    *   倒计时在每次插件刷新时重新计算，精度取决于插件的刷新间隔 (`interval`)。
